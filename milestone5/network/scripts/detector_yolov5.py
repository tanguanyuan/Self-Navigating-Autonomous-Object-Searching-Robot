...
class Detector:
    def __init__(self, ckpt, use_gpu=False):
        self.args = args
        self.model = DetectMultiBackend('network/scripts/best5_7.pt')
    ...
    def validate_pred(self, xyxy, conf, cls):
        ...
        if cls == 1: # redapple
            if height_width_ratio < 0.940 or height_width_ratio > 1.300: #0.958 1.284
                return False
        elif cls == 2: # greenapple
            if height_width_ratio < 0.770 or height_width_ratio > 0.980: #0.792, 0.971
                return False
        elif cls == 3: # orange
            if height_width_ratio < 0.909 or height_width_ratio > 1.080: # 0.981 1.067
                return False
        elif cls == 4: # mango
