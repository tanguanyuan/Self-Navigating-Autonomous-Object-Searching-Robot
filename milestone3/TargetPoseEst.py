...
def merge_estimations(target_map):
    ...
    if len(redapple_est) > num_per_target:
        redapple_est = np.mean(redapple_est[0:], axis=0)
        redapple_est = [redapple_est]
    if len(greenapple_est) > num_per_target:
        greenapple_est = np.mean(greenapple_est[0:], axis=0)
        greenapple_est = [greenapple_est]
    if len(orange_est) > num_per_target:
        orange_est = np.mean(orange_est[0:], axis=0)
        orange_est = [orange_est]
    if len(mango_est) > num_per_target:
        mango_est = np.mean(mango_est[0:], axis=0)
        mango_est = [mango_est]
    if len(capsicum_est) > num_per_target:
        capsicum_est = capsicum_est[0:] #np.mean(capsicum_est[0:num_per_target], axis=0)
