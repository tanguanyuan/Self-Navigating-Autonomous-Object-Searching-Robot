...
def estimate_pose(base_dir, camera_matrix, completed_img_dict):
    ...
    for target_num in completed_img_dict.keys():
        ...
        dist = true_height/box[3]*focal_length
        u0 = camera_matrix[0][2]
        new_theta = robot_pose[2] + np.arctan2((box[0]-u0), focal_length)
        x = robot_pose[0]+np.cos(new_theta)*dist
        x = x[0]
        y = robot_pose[0]+np.sin(new_theta)*dist
        y = y[0]
        target_pose = {'x': x, 'y': y}
...
def merge_estimations(target_map):
    ...
    if len(redapple_est) > num_per_target:
        redapple_est = np.mean(redapple_est[0:num_per_target], axis=0)
    if len(greenapple_est) > num_per_target:
        greenapple_est = np.mean(greenapple_est[0:num_per_target], axis=0)
    if len(orange_est) > num_per_target:
        orange_est = orange_est[0:num_per_target]#np.mean(orange_est[0:num_per_target], axis=0)
    if len(mango_est) > num_per_target:
        mango_est = mango_est[0:num_per_target]#np.mean(mango_est[0:num_per_target], axis=0)
    if len(capsicum_est) > num_per_target:
        capsicum_est = capsicum_est[0:num_per_target]#np.mean(capsicum_est[0:num_per_target], axis=0)
